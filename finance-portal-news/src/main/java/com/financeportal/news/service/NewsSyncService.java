...
import com.financeportal.news.client.RssIntegrationClient;
import com.financeportal.news.client.TranslationClient;
...
public class NewsSyncService {
    ...
    /**
     * Ilk senkron denemesi tamamlandi mi. Basarili olmasi SART DEGIL — hata durumunda
     * da isaretlenir, cunku soru "haber geldi mi" degil "servis calismaya basladi mi".
     * Ileride hazir-olma (readiness) kontrolune baglanabilir.
     */
    private final AtomicBoolean syncCompletedOnce = new AtomicBoolean(false);

    public boolean hasCompletedSync() {
        return syncCompletedOnce.get();
    }
    ...
    public void fetchAndCacheNews() {
        try {
            ...
            // Backend'de burada bir "acilis ilerleme banneri"na kaydolunuyordu.
            // Ayri bir serviste onun karsiligi yok; yerine servisin KENDI durumu
            // tutuluyor: ilk senkron tamamlandi mi? Bu, hata olsa bile isaretlenir —
            // amac "veri geldi mi" degil, "deneme bitti mi".
            syncCompletedOnce.set(true);
            log.info("[{}] Haber senkronizasyonu tamamlandı.", TASK_NAME);
        }
    }
