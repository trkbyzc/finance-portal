    ...
    return (
        <div className="bg-surface border border-border rounded-2xl overflow-hidden">
            <div className="sticky top-0 z-10 bg-surface-2/95 backdrop-blur-sm border-b border-border px-4 py-2 flex items-center justify-between">
                <span className="font-bold text-sm capitalize">{fmtDateHeader(day, locale)}</span>
                <span className="text-[10px] text-text-muted">
                    {t('eventsLabel', { count: events.length })}
                </span>
            </div>
...
function EventRow({ event: e, locale }) {
    ...
    return (
        <div className="px-4 py-3 hover:bg-surface-hover transition flex items-center gap-3 text-sm">
            <div className="font-mono text-text-muted w-14 shrink-0">{fmtTime(e.time, locale)}</div>
            ...
            <div className="hidden md:flex items-center gap-4 text-right shrink-0">
                <div className="w-20">
                    ...
                    <div className="font-mono text-xs">{fmtNum(e.previous, e.unit, locale)}</div>
                </div>
                <div className="w-20">
                    <div className="text-[9px] text-text-muted uppercase tracking-wider">{t('cols.estimate')}</div>
                    <div className="font-mono text-xs">{fmtNum(e.estimate, e.unit, locale)}</div>
                </div>
                <div className="w-20">
                    <div className="text-[9px] text-text-muted uppercase tracking-wider">{t('cols.actual')}</div>
                    <div className={`font-mono text-xs font-bold ${actualColor}`}>{fmtNum(e.actual, e.unit, locale)}</div>
                </div>
            </div>
        </div>
    );
